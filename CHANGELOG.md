...
- Зафіксовано 24/7, паралельність, безкоштовні AI-моделі, Angular Web + CLI, Telegram та мінімальне зберігання результатів.
- Документаційна зміна; тести не запускалися.

## ПРАВИЛЬНА НАЗВА: 0.0.2.3
- Доповнено AGENTS.md правилами пакетної перевірки ir, обмеженого делегованого закриття, запиту прав і погодженої консолідації залишку.
- Уточнено єдиний API лабораторії для CLI та окремої вебпанелі; технічне ТЗ — Issue #107.
- Документаційна зміна, без запуску тестів.

## ПРАВИЛЬНА НАЗВА: 0.0.2.2
- Оновлено AGENTS.md: обмеження обсягу лише нових Issues, збереження всіх наявних Issues без змін та незалежність від конкретного AI-інструмента.
- Додано governance автономної лабораторії: тип lab, погодження lab/var → i, незалежна перевірка ir, розмежування прав та заборона самостійної публікації.
- Документаційний коміт за прямим дорученням власника; тести не запускалися.
...
- Paths consistency (#53): `health_checks.py`, `backup_service.py`, `worker/service.py` — єдиний `_PROJECT_ROOT` для шляхів backup/redis замість хардкоду `/data/backups`, `/var/lib/redis`
