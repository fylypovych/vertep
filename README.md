...
Web updates are enabled by the Ubuntu CORE installer (`WEB_UPDATE_ENABLED=true`) and restricted to administrators. Releases are fetched only from `VERTEP_UPDATE_SERVER` (default `https://update.vertep.ai`) and their detached signature is verified with `UPDATE_PUBLIC_KEY` before a package is accepted. The updater enters maintenance mode, drains active work, creates application, job, configuration, migration, and PostgreSQL backups, then applies the package. The release only counts as installed once the health gate passes: PostgreSQL with every packaged migration applied, a Redis read-write round trip, the API reporting the new version, the Web UI, Ollama when configured, a dispatchable Worker, every registered node in `READY`, and, on Worker nodes, GPU, driver, CUDA, free VRAM, ComfyUI and a self-test younger than `WORKER_SELF_TEST_MAX_AGE_SECONDS`. A second gate runs after dispatch resumes and additionally requires `NORMAL` state, an unpaused queue and no job left in `WAITING_FOR_SYSTEM`. Any failed check triggers an automatic rollback; a durable update phase permits recovery after a power loss. The same report is available from `GET /api/system/health/full` and `vertep health-gate`. The systemd timer checks for releases every six hours. GitHub credentials and repository access are never required on an installed node.
...
- `GET /api/system/health/full?mode=post-update|final`
