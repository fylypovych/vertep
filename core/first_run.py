...
def complete_setup(name: str, username: str, password: str, confirmation: str,
    ...
    if web_domain and web_domain.strip():
        ...
        if env_path.exists():
            ...
            if "WEB_DOMAIN=" in text:
                text = "\n".join(line for line in text.splitlines() if not line.startswith("WEB_DOMAIN="))
                text += row
            else:
                text += row
            env_path.write_text(text, encoding="utf-8")
        else:
            env_path.write_text(row, encoding="utf-8")
    if node_credentials:
    ...
    manifest["manifest_sha256"] = hashlib.sha256(json.dumps(
        {key: item for key, item in value.items() if key != "administrator"},
        sort_keys=True, separators=(",", ":")).encode()).hexdigest()
    core_certificate_path = Path(config_root()) / "tls" / "vertep.crt"
    if core_certificate_path.is_file():
        manifest["core_certificate"] = core_certificate_path.read_text(encoding="utf-8")
    return manifest
