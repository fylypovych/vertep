...
| Core/Worker health checks | Частково | Спільний health gate (`core/health_gate.py`) перевіряє PostgreSQL зі станом міграцій, Redis read/write, API/версію, Web UI, Ollama, Dispatcher, GPU/driver/CUDA/VRAM/ComfyUI, свіжість self-test і готовність усього флоту; ще потрібні production image tests і publisher/artifact assertions на реальному залізі. |
...
| Автоматичний rollback | Частково | Application rollback атомарно повертає immutable release, перевіряє backup checksums і автоматично відновлює PostgreSQL dump; тригером є post-update та final health gate, а не факт запуску контейнера. Compatibility рішення і health check відновленої версії ще потребують integration qualification. |
