...
def test_job_create_returns_required_fields():
    ...
    assert job["status"] in {"NEW", "SCRIPT_QUEUED", "SCRIPT_PENDING_APPROVAL"}
...
def test_integrations_includes_publisher_channels():
    client = _client()
    resp = client.get("/api/integrations")
    assert resp.status_code == 200
    data = resp.json()
    assert "publisher" in data
    platforms = data["publisher"]
    assert isinstance(platforms, dict)
    assert "youtube" in platforms
    assert "configured" in platforms["youtube"]
